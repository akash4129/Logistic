...
var method=function(el){el=$(el);return el.nodeName.toLowerCase()==tagName.toLowerCase();};return YAHOO.Smb.Asteroids.Extensions.Util.getAncestorBy(node,method);}};YAHOO.namespace("Smb.Asteroids.Extensions.SlideShow");(function(){var YSAEU=YAHOO.Smb.Asteroids.Extensions.Util;var SS=YAHOO.Smb.Asteroids.Extensions.SlideShow;var imageData=YAHOO.Smb.Asteroids.ImageData;var ua=navigator.userAgent.toLowerCase();var isIE=!ua.isOpera&&ua.indexOf('msie')>-1;var Constants={'CLASS_DISPLAYNONE':'displayNone','CLASS_DIMMED':'dimmed','CLASS_ALLOWZOOMIN':'yssImg_allowZoomIn','CLASS_YSSIMGSINGLE':'yssImg_single','CLASS_RTEIMG':'yssImg_RTE','CLASS_MODFIELD':'modfield','CLASS_MODBDCONTAINER':'module_bd_container','CLASS_YSSDKIMG':'yssDKImg','CLASS_ZOOMINCTRL':'imgZoomInCtrl'};SS.init=function(){SS.ZoomIn.init();};SS.ZoomIn={interactors:new Array(),viewer:null,addInteractor:function(interactor){interactor.onZoomIn.subscribe(this.onZoomIn,this,true);this.interactors.push(interactor);},init:function(){if(this.interactors.length===0){this.addInteractor(SS.HoverInteractor);this.addInteractor(SS.ThumbClickInteractor);}
